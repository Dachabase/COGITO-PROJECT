...
from schemas.user import generate_credentials,generate_HR_credentials, hash_password,get_current_user,authenticate_user,create_access_token, ACCESS_TOKEN_EXPIRE_MINUTES, SECRET_KEY, ALGORITHM,pwd_context,oauth2_scheme,generate_staff_id
...
async def read_own_items(current_user: UserInDB = Depends(get_current_user)):
    if current_user.role == "staff":
        ...
        return [{"item_id": 2, "owner": current_user}]
    elif current_user.role == "HR":
        return [{"item_id": 3, "owner": current_user}]
    else:
...
async def update_staff(staff_id: str, staff: Staff, current_user: UserInDB = Depends(get_current_user)):
    ...
    staff_data = user_collection.find_one({"_id": staff_id})
    if not staff_data:
    ...
    user_collection.update_one({"_id": staff_id}, {"$set": staff_data})
    
    return {"message": "Staff details updated successfully"}

@user.delete("/staff/{staff_id}")
async def delete_staff(staff_id: int, current_user: UserInDB = Depends(get_current_user)):
    if staff_id != current_user.staff_id:
    ...
    user_collection.delete_one({"_id": staff_id})
